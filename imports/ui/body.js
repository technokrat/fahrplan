...
import { AvailableStations } from '../api/availablestations.js'
import { Connections } from '../api/connections.js'
import { Status } from '../api/status.js'
import { Meteor } from 'meteor/meteor'

import { UPDATE_PERIOD } from '../init/parameters.js'
...
Template.body.onRendered(() => {
  ...
  $(document).on('keydown', e => {
    if (e.keyCode == 27) Session.set('route', '')
  ...
  connectionBoard = new connection_board.ConnectionBoard(
    $('#connection_board')[0],
    resized
  )
...
Template.overlay.helpers({
  failure: function () {
    ...
    if (!Meteor.status().connected) return 'Bad connection to webserver'
    else if (status && status.date && status.date <= Date.now() - 60000)
      return 'No updates from HAFAS!'
    else if (
      stationSubscription.ready() &&
      !Stations.findOne({
        ibnr: Session.get('station_ibnr'),
      })
    )
...
Template.body.helpers({
  ...
  isOverlay: function () {
    ...
    if (!Meteor.status().connected) return true
    else if (status && status.date && status.date <= Date.now() - 60000)
      return true
    else if (
      stationSubscription.ready() &&
...
Template.body.onCreated(function () {
  ...
  if (!station_ibnr) station_ibnr = localStorage.getItem('station_ibnr')
  if (!station_ibnr) station_ibnr = '8591123'

  Session.set('station_ibnr', station_ibnr)
  localStorage.setItem('station_ibnr', station_ibnr)

  Tracker.autorun(function () {
    stationSubscription = Meteor.subscribe(
      'stations',
      Session.get('station_ibnr'),
      Session.get('connection_count')
    )
    Meteor.subscribe(
      'connections',
